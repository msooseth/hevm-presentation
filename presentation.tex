...
\title[hevm]{
hevm, new frontiers in symbolic execution of EVM bytecode}
\author[dxo, Soos, Blicha, Paraskevopoulou]
{dxo, \emph{Mate Soos}, Martin Blicha, Zoe Paraskevopoulou,\\
    with help from the Echidna team, esp. Gustavo Grieco
}
\institute[Argot]{\large Argot Collective (\url{https://argot.org})}
\date{19th of November 2025, Devconnect Argentina}
...
\item \textbf{Certora Prover}: Based on backwards exploration and weakest
    precondition computation, with its own specification language
\item \textbf{KEVM}: K-framework based, allows to ``break out'' into K to prove
    lemmas
\item \textbf{halmos}: Most similar to hevm, written in python, using z3 as its
    IR
...
% \begin{itemize}
% \item Started $\approx7$ years ago as part of dapptools, but is now a
%     standalone tool
...
\section{hevm's Symbolic Executor}
...
\item Can run on \textbf{any point in blockchain history} via RPC
\item Runs an \textbf{SMT solver} to find input to reach counterexamples
\end{itemize}
\bigskip

Limitations:
\begin{itemize}
\item Cannot deal with \textbf{symbolic gas} or \textbf{symbolic code} other than
    by overapproximation
...
% DEVCON maybe add?
 ...
 % The IR is translated in a straightforward manner to SMT. Let's say the
 % expression is:
...
% \begin{itemize}
%     \item I can steal the money due to e.g. overflow/reentrancy/logical
%         error/etc bug
%     \item I can lock in the money due to liveness issues
%     \item I can exploit incorrect rounding in online exchanges to get better
%         prices
%     \item I can borrow money to buy some asset, dump it in an automated market
%         maker pushing its price to zero, then buy everything for zero.
%     \item Frontrunning: I can observe a transaction, see that it makes money,
%         and replace the recipient of the transaction with my own address, so I
...
% \bigskip
% And the list goes on. Some of these are ''clearly`` bugs. Some of them are
% abuse of systems (frontrunning). Most interesting oners are games played
% according to the rules, but not according to the spirit.
% \end{frame}
...
%     \item We provide an interface where it's possible to specify
%         pre/post-conditions
%     \item Separate tool is in the works that adds pre/post conditions to
%         prevent the above
% \end{itemize}

% \bigskip
% This way we allow others to be opinionated, but we ourselves remain
% unopinionated.
% \end{frame}

\begin{frame}{Example: The Great Verification of ABDK Math 64.64}
    \centering
    An effort to verify mathematical properties in ABDK Math 64x64 using echidna/hevm.\footnote{
    ABDK Math 64x64: Library of mathematical functions operating with signed 64.64-bit fixed point numbers.}
    \vfill

    \includegraphics[scale=0.13]{abdk-table-1}\quad\includegraphics[scale=0.13]{abdk-table-2}

    \textbf{Verified}: 34/57 $\mathbf{(59.65\%)}$

    \url{https://github.com/gustavo-grieco/abdk-math-64.64-verification}
\end{frame}

\begin{frame}[fragile]{\((x + y) + z = x + (y + z)\)}

    \centering
    \begin{Verbatim}[framerule=0.2mm, framesep=2mm,fontsize=\footnotesize]
    ...
    \end{Verbatim}
    \rule{0.99\linewidth}{0.4pt}
    \vfill
    \begin{Verbatim}[framerule=0.2mm, framesep=2mm,fontsize=\footnotesize]
    [RUNNING] prove_add_associative(int128,int128,int128)
    Exploring call prefix 0x6727ee5c
    Flattening expression
    Exploration finished, 14 branch(es) to check in call prefix 0x6727ee5c
    Keccak preimages in state: 0
    Checking for reachability of 1 potential property violation(s) in call prefix 0x6727ee5c
    SMT result: Qed
    \end{Verbatim}

\end{frame}

% \begin{frame}[fragile]{Finding a bug in ABDK Math 64.64's \texttt{prove\_inv\_identity}}
% \begin{Verbatim}
% hevm test --root abdk-math-64.64-verification --max-depth 70 \\
%     --solver bitwuzla --match prove_inv_identity
% \end{Verbatim}

% Output:
% \begin{Verbatim}
% Exploration finished, 58 branch(es) to check in call prefix 0xacce097a
% [FAIL] prove_inv_identity
% Counterexample:   [validated]
%    calldata: prove_inv_identity(-108003701288662363396026832958930103201)
% [WARNING] hevm was only able to partially explore the test prove_inv_identity
...
\begin{frame}{Unique Selling Points of hevm}
It is \textbf{easy to use}:
\begin{itemize}
    \item Uses forge-std cheatcodes for easy test setup
    % \item Can be used as a \textbf{standalone tool}, no need for pypi/npm/etc
    \item Works on any EVM contract deployed on any chain supported by an RPC node
    \item No need to recompile contracts, or have their source code
\end{itemize}
% \texttt{hevm symbolic --rpc-url RPC\_URL --contract ADDR --match FUNCTION\_NAME}
\smallskip

Can be used as a \textbf{library}:
\begin{itemize}
    \item Echidna uses hevm's concrete execution for its fuzzer
    \item New version of Echidna will be able to also use
        hevm's symbolic execution engine to complement its fuzzer
    \item Such combined approach allows it to combine the
        ``best of both worlds''
\end{itemize}
\smallskip

Can be used as an \textbf{equivalence checker}:
\begin{itemize}
    \item Used by Act, a declarative smart contract language, to check
        equivalence of the declarative description
        against the actual compiled contract
    \item Used by Solidity's fuzzer infrastructure to check compiler
        optimizations
\end{itemize}
\end{frame}

\begin{frame}[fragile=singleslide]{Equivalence Checking with hevm}
hevm compares all externally visible behaviours of two contracts:
\begin{itemize}
    \item state
    \item return values
    \item reverts
\end{itemize}
but not events, gas usage, or intermediate states and values.
\bigskip
...
\section{Improvements Since Devcon 2024}
\begin{frame}{Improvements since DevCon: Highlights}
\begin{itemize}
    \item Overapproximation for \texttt{STATICCALL}, \texttt{BALANCE},
        \texttt{EXTCODESIZE}, and \texttt{EXTCODEHASH}
    \item SMT solver now used to concretize symbolic expressions and continue execution
    \item Multiple solutions for symbolic expressions via iterative SMT solving
    \item Improved aliasing for symbolic and concrete addresses
    \item Constant propagation for symbolic values
    \item Branch depth and width can be limited
    \item Major CLI improvements for better UX
\end{itemize}
\end{frame}

\begin{frame}{Improvements since DevCon: Highlights cont.}
\begin{itemize}
    \item Counterexamples are now validated via concrete execution
    \item Mapping of warnings and errors to source lines
    \item Better RPC caching\&mocking framework via \texttt{--cache-dir}:
      persistent shared cache, single HTTPS session
    \item Allowing to restrict unknown address resolution to deployed contracts
        via \texttt{--only-deployed}
    \item Keccak computations extracted and asserted for correctness
    \item Better collection of storage reads for counterexamples
    \item Upcoming: queued symbolic execution for more robust exploration of
        large contracts
\end{itemize}
\end{frame}

\begin{frame}{Improvements since DevCon: Minor Things}
\begin{itemize}
    \item Robust handling of invalid cheatcodes/precompiles
    \item Geth-compatible JSONL traces via \texttt{--json-trace}
    \item Updated forge and forge-std versions
    \item Simplified printed expressions in symbolic mode
    \item More robust calldata decoding, even with trailing bytes
    \item More aggressive calldata shrinking
    % \item Minor bugfixes to IR simplification
    % \item Faster \texttt{word256Bytes} and \texttt{word160Bytes}
\end{itemize}
\end{frame}

\begin{frame}{Improvements since DevCon: Minor Things cont.}
\begin{itemize}
    \item New rewrite and simplification rules
    \item Read bytecode directly from files, supporting large contracts
    \item Better handling of symbolic \texttt{block.number} in library mode
    \item Optimized memory representation and operations
    \item Many bugfixes: infinite recursion, out-of-bounds reads, duplicate
        assertions, and more
    \item Test suite improvements and faster test running
    \item Documentation and tutorial updates
\end{itemize}
\end{frame}

\begin{frame}{Overall Effect of Improvements}
\begin{itemize}
    \item Better first-time user experience
    \item Easier-to-understand counterexamples
    \item More robust handling of real-world contracts
\end{itemize}
\bigskip
...
Upcoming changes:
\begin{itemize}
    \item Queued symbolic execution for better exploration of large contracts
    \item Better separation of symbolic/concrete execution modes
    \item Dumping execution traces' calldata for fuzzers to improve their corpus

        e.g. Echidna -- but see Alex's talk later today!
\end{itemize}
\end{frame}

\begin{frame}{Conclusions}
\begin{itemize}
    \item Fully-featured symbolic\&concrete execution engine for EVM bytecode
    \item Easy-to-use: test setup via forge cheatcodes, standalone CLI
    \item Runs on any point in chain history via RPC
    \item Highly optimized IR with extensive simplifications
    \item Tight integration with an SMT solver for finding buggy inputs
    \item Used in practice by multiple tools and projects
    \item Library that can be integrated into other tools
    \item Actively maintained open-source project
\end{itemize}
\bigskip

Repository: \url{https://github.com/argotorg/hevm/}
\bigskip

User guide: \url{https://hevm.dev/}
% Code: \qquad \qrcode[height=0.5in]{https://github.com/argotorg/hevm/}
% Research Paper: \qquad \qrcode[height=0.5in]{https://dl.acm.org/doi/10.1007/978-3-031-65627-9_22}
