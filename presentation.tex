...
\item Can be set up to run at any point in time (blockhain history), through RPC
...
Limitations:
\begin{itemize}
\item Cannot deal with symolic gas other than ignoring it
\item Symbolic offset/size memory copy is not yet implemented
\item Because it runs on bytecode, it needs to recover variable bitwidths, loops, etc.
\end{itemize}
\end{frame}
...
\end{Verbatim}
This can later be collapsed, e.g. in case the variables are concretized. We understand \texttt{(AbstractBuf "var")} and \texttt{(ConcreteBuf "value")}
\bigskip
...
Notice that geth, the normal EVM concrete executor is incredibly slow. Hence a transpiled IR could achive serious speedup.
