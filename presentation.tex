...
HEVM can help with finding ways to trigger \emph{all} known bad states and validate that all invariants \emph{always} hold.
...
Fuzzing nevers find this edge-case. Symbolic execution always finds it.
\bigskip 
...
An example testing strategy could look like:
\begin{itemize}
\item Unit tests, component tests, integraton tests, user-acceptance tests
\item Manual \& exploratory tests (e.g. penetration testing)
\item Known bad state \& invariant checking through fuzzing \& symbolic execution
\end{itemize}
\bigskip
